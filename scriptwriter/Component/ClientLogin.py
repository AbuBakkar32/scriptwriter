...
class ClientLogin(object):
    def __init__(self, request):
        ...
        self.err_email = {"err":["Invalid Email Information!!!","alert alert-danger","margin-bottom:1%"],
    ...
    def login(self): 
        ...
        if request.method == "POST":
            ...
            if page == "loginpage":
                ...
                if not user.exists(): return render(request, "login.html", self.err_email)
                else:
                    ...
                    if password == datasuit['password']:
                        ...
                        cookie =  str(request.META.get('CSRF_COOKIE')) #for ip-address #(request.META.get('HTTP_X_FORWARDED_FOR')).split(',')[0]
                        f.season = "%s"%(str(cookie))
                        f.save()
                        return HttpResponseRedirect("/client-home")
                    else: return render(request, "login.html", self.err_password )
            ...
            else: return render(request,"login.html", self.content_context)
