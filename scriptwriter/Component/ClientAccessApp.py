...
class ClientAccessApp(object):
    ...
    def logout(self):
        ...
        if request.method == "GET":
            ...
            if user.exists():
                ...
                f.season = "0.0"
                f.save()
                return render(request, "login.html" , self.err_logout)
    ...
    def forget_password(self):
        ...
        if request.method == "GET":
            sea = str(request.META.get('CSRF_COOKIE'))
            user = Client.objects.filter(season=sea)
            if user.exists(): return HttpResponseRedirect("/client-home")
            else: return render(request,"forget_password.html", self.contentContext)
        ...
        if request.method == "POST":
            ...
            if page == "passwordsubmit":
                ...
                if password1 == password2:
                    ...
                    datasuit = {}
                    datasuit["err"] = ["New password doesn't match each other, please take note of case sensitivity !!!",
    ...
    def Nforget_password(self):
        ...
        if request.method == "GET":
            ...
            if str(p).startswith("<QuerySet []>") == False: return HttpResponseRedirect("/client-home")
            elif str(p).startswith("<QuerySet []>") == True: return render(request, "forget_password.html", self.contentContext)
            else: return render(request,"forget_password.html", self.contentContext)
        ...
        if request.method == "POST":
            ...
            if page == "apply":
                ...
                rpv = generateid("passwordreset")
                casted_email = Client.objects.filter(email=email)
                if str(casted_email) == "<QuerySet []>": return render(request,"forget_password.html", self.errResetFailed)
                f = Client.objects.get(email=email)
                f.resetPasswordValue = str(rpv)
                f.save()
    ...
    def Nclient_verify_email(self, slug1, slug2):
        ...
        if request.method == "GET":
            ...
            if str(p) == "<QuerySet []>": return HttpResponseRedirect("/login")
            else:
    ...
    def Nclient_resetpassword(self, slug1):
        ...
        if request.method == "GET":
            ...
            if str(p) == "<QuerySet []>": return HttpResponseRedirect("/forget-password")
            else:
                datasuit = arrayDBData(p, 'Client')
                datasuit["uservalue"] = reset_password_value
